...
def rk4_leg(f0):
    ...
    num_g1 = (-np.sin(teta1-teta2)*(m2*l2*(teta21**2)
                    +m2*l1*(teta11**2)*np.cos(teta1-teta2))
                    -g*((m1 +m2)*np.sin(teta1) - m2*(np.sin(teta2)*np.cos(teta1-teta2))))
    den_g1 = l1*(m1+m2*((np.sin(teta1-teta2))**2))

    g1 = num_g1/den_g1


    num_g2 = (np.sin(teta1-teta2)*((m1+m2)*l1*(teta11**2)
                    +m2*l2*(teta21**2)*np.cos(teta1-teta2))
                    +g*((m1 +m2)*(np.sin(teta1)*np.cos(teta1-teta2)-np.sin(teta2))))
